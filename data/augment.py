...
def augmentation_train(crop_size=32, resize=0, preprocess=True, preprocess_arg=None, no_augment=False):
    augment_stack = []
    if preprocess:
        if preprocess_arg:
...
def augmentation_test(crop_size, resize=0, preprocess=True, preprocess_arg=None, no_augment=False):
    augment_stack = []
    if preprocess:
        if preprocess_arg:
            augment_stack.append(CorrectionFilterEqualize(**preprocess_arg))
