...
from model.vgg_face_module import VGGBlock, VGGResBlock
...
class VGGFaceNet(nn.Module):
    def __init__(self, input_channel=3, num_of_classes=1000,
        ...
        self.conv0 = VGGBlock(input_channel, 64, 3, blocks[0], init=init, bn=bn)
...
class VGGFaceResNet(nn.Module):
    def __init__(self, input_channel=3, num_of_classes=1000,
                 blocks=[2, 2, 3, 3, 3], dropout=0.5,
                 init=nn.init.kaiming_uniform_, bn=True):
        super(VGGFaceResNet, self).__init__()
        self.pool = nn.MaxPool2d(2)
        self.conv0 = VGGResBlock(input_channel, 64, 3, blocks[0], init=init, bn=bn)
        self.conv1 = VGGResBlock(64, 128, 3, blocks[1], init=init, bn=bn)
        self.conv2 = VGGResBlock(128, 256, 3, blocks[2], init=init, bn=bn)
        self.conv3 = VGGResBlock(256, 512, 3, blocks[3], init=init, bn=bn)
        self.conv4 = VGGResBlock(512, 512, 3, blocks[4], init=init, bn=bn)
        self.fc_conv1 = nn.Conv2d(512, 4096, 7)
        init(self.fc_conv1.weight)
        self.fc_conv2 = nn.Conv2d(4096, 4096, 1)
        init(self.fc_conv1.weight)
